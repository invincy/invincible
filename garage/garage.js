...
function render(){
 ...
 syncCurrentKm();$("vehicleType").textContent=v.type+" · "+v.fuelType;$("vehicleName").textContent=v.name;$("vehicleMeta").textContent=v.registration||"Registration not added";$("currentKm").textContent=Number(v.odometer||0).toLocaleString("en-IN")+" km";const autoKind=/bike|motorcycle|scooter/i.test(v.type+" "+v.name)?"bike":"car",art=v.art&&v.art!=="auto"?v.art:autoKind,theme=v.theme&&v.theme!=="auto"?v.theme:(autoKind==="bike"?"green":"silver");$("vehicleArt").dataset.kind=art;document.body.dataset.theme=theme;
...
$("vehicleForm").onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),oldItem=editing.id?state.vehicles.find(x=>x.id===editing.id):null,item={id:editing.id||newId(),name:f.get("name").trim(),type:f.get("type"),registration:f.get("registration").trim(),fuelType:f.get("fuelType"),theme:f.get("theme")||"auto",art:f.get("art")||"auto",odometer:Number(f.get("odometer"))};upsert("vehicle",item);activeId=item.id;if(!oldItem||Number(oldItem.odometer)!==item.odometer){const existing=state.odometerReadings.find(x=>x.vehicleId===item.id&&x.date===today());if(existing)existing.odometer=item.odometer;else state.odometerReadings.push({id:newId(),vehicleId:item.id,date:today(),odometer:item.odometer})}syncCurrentKm();render();await save();e.currentTarget.reset();$("vehicleDialog").close()};
...
$("deleteVehicle").onclick=async()=>{const v=vehicle();if(!v||!confirm("Delete "+v.name+" and all its records?"))return;state.vehicles=state.vehicles.filter(x=>x.id!==activeId);["odometerReadings","fuel","services","reminders"].forEach(key=>state[key]=state[key].filter(x=>x.vehicleId!==activeId));activeId=state.vehicles[0]?.id||"";render();await save()};
document.body.addEventListener("click",async e=>{const b=e.target.closest("[data-action]");if(!b)return;const kind=b.dataset.kind,id=b.dataset.id;if(b.dataset.action==="edit"){openDialog(kind,id);return}if(!confirm("Delete this entry?"))return;const key=kind==="km"?"odometerReadings":kind==="service"?"services":kind;state[key]=state[key].filter(x=>x.id!==id);syncCurrentKm();render();await save()});
