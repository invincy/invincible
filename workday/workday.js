...
function cleanSentence(value){const text=(value||"").trim().replace(/[.]+$/,"");if(!text)return"";return text.charAt(0).toUpperCase()+text.slice(1)+"."}
function reportSentence(task,note=""){
 const action=(task.action||"").trim(),subject=(task.subject||"").trim(),key=action.toLowerCase();let line="";
 if(key==="mailed")line=subject?`Mailed to ${subject}`:"Mailed";
 else if(key==="telephonic talk"||key==="had telephonic talk")line=subject?`Had telephonic talk with ${subject}`:"Had telephonic talk";
 else if(["reviewed","confirmed","processed"].includes(key))line=[action,subject].filter(Boolean).join(" ");
 else line=[action,subject].filter(Boolean).join(" ")||(task.title||"").trim()||(task.outcome||"").trim();
 const detail=(note||taskDetail(task)).trim().replace(/[.]+$/,"");return cleanSentence(line+(detail?` — ${detail}`:""));
}
...
function buildReport(){const items=tasksForDay(),section=(title,list,note)=>list.length?`${title}\n${list.map(task=>`• ${reportSentence(task,note(task))}`).join("\n")}\n\n`:"";let report=`Daily Work Report — ${new Intl.DateTimeFormat("en-GB").format(new Date(selectedDate+"T12:00:00"))}\n\n`;report+=section("✅ Jobs Completed",items.filter(t=>t.status==="done"),()=>"");report+=section("🔄 Jobs In Progress",items.filter(t=>t.status==="progress"),()=>"");report+=section("⏳ Pending / Backlog",items.filter(t=>t.status==="pending"||t.status==="todo"),t=>t.pendingReason||"");const next=items.filter(t=>t.status!=="done").map(t=>t.nextAction||(t.status==="progress"?`Finish ${taskLabel(t)}`:`Complete ${taskLabel(t)}`));if(next.length)report+=`📌 Next Working Day\n${next.map(x=>`• ${cleanSentence(x)}`).join("\n")}\n`;return report.trim()||`Daily Work Report — ${selectedDate}\n\nNo work recorded.`}
