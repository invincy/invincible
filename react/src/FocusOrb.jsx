 ...
 useEffect(()=>{
  ...
  try{
   ...
   const render=timestamp=>{
    ...
    gl.useProgram(dots);gl.uniformMatrix4fv(dotProjection,false,projection);gl.uniform1f(dotRatio,ratio);gl.uniform1f(dotTime,time);gl.uniform1f(dotStyle,palette.style);gl.uniform3fv(dotColorA,palette.a);gl.uniform3fv(dotColorB,palette.b);gl.uniform2f(dotRotation,yaw+hoverX*hoverStrength*.10,pitch-hoverY*hoverStrength*.10);gl.uniform3f(dotInfluence,hoverX,hoverY,hoverStrength);gl.bindBuffer(gl.ARRAY_BUFFER,particleBuffer);gl.enableVertexAttribArray(dotPosition);gl.vertexAttribPointer(dotPosition,3,gl.FLOAT,false,20,0);gl.enableVertexAttribArray(dotAppearance);gl.vertexAttribPointer(dotAppearance,2,gl.FLOAT,false,20,12);gl.enable(gl.BLEND);gl.blendFunc(gl.SRC_ALPHA,gl.ONE);gl.depthMask(false);gl.drawArrays(gl.POINTS,0,particleCount);gl.depthMask(true);
